...
def create_training_material(
    token: str,
    lesson_id: str,
    training_material_name: str,
    feedback_urls: list[Optional[str]],
    step_times: list[int],
    question_sets: list[list[str]] = None,
    answer_sets: list[list[str]] = None,
    ...
    for part, (questions, answers, generator_path) in enumerate(
        zip_longest(question_sets, answer_sets, generator_paths)
    ):
        if not (questions and answers) and not generator_path:
            raise ValueError(
                "Either question-answer pairs or generator must be provided"
            )
        data[f"{FrontendConstants.QUESTIONS}_{part}"] = questions
        data[f"{FrontendConstants.ANSWERS}_{part}"] = answers
    files = dict(
        (FrontendConstants.HTML_FILE.format(index), (html_path.name, html_path.open()))
    ...
    files = {
        ...
        **dict(
            (
                FrontendConstants.PY_FILE.format(index),
                (generator_path.name, generator_path.open()),
            )
            for index, generator_path in enumerate(generator_paths)
            if generator_path
        ),
    }
